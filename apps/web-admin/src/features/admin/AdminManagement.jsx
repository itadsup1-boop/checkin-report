...
export default function AdminManagement({ groups = [] }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white backdrop-blur-xl border border-slate-100 p-6 rounded-3xl shadow-xl">
        <div>
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-gradient-to-tr from-blue-500 to-blue-600 rounded-2xl shadow-lg shadow-blue-500/20">
              <Shield className="w-6 h-6 text-white" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Quản lý Tài khoản Admin</h2>
              <p className="text-slate-500 text-sm mt-0.5">Phân quyền quản lý các nhóm Telegram cho từng tài khoản Admin</p>
            </div>
          </div>
        </div>
      ...
      {/* Admins List Table */}
      <div className="bg-white backdrop-blur-xl border border-slate-100 rounded-3xl overflow-hidden shadow-xl">
                  ...
                  return (
                    <tr key={admin.id} className="hover:bg-slate-50 transition-colors">
                      <td className="py-4 px-6 font-semibold text-slate-900 flex items-center gap-2">
                        <div className="w-8 h-8 rounded-full border border-blue-200 bg-blue-50 flex items-center justify-center text-blue-700 font-bold">
                      ...
                      <td className="py-4 px-6">
                        {isSuper ? (
                          <span className="text-slate-500 italic text-xs">Toàn bộ nhóm (Tất cả)</span>
                          ...
                          <div className="flex flex-wrap gap-1.5 max-w-md">
                            {adminAssignedGroups.map(gId => {
                              ...
                              return (
                                <span key={gId} className="px-2.5 py-0.5 bg-slate-50 border border-slate-200 text-slate-600 rounded-lg text-xs">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-900 focus:outline-none focus:border-blue-500 disabled:opacity-50"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  minLength={editingAdmin && !formData.password ? undefined : 12}
                  maxLength={128}
                ...
                />
                <p className="mt-1 text-[11px] text-slate-500">Tối thiểu 12 ký tự và không chứa tên đăng nhập.</p>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="px-4 py-2.5 border border-slate-200 bg-white hover:bg-slate-50 text-slate-700 rounded-xl text-sm font-medium transition-colors"
                >
                  Hủy
                </button>
                <button
                  type="submit"
                  className="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white rounded-xl text-sm font-bold shadow-lg shadow-blue-500/25 transition-all"
                >
