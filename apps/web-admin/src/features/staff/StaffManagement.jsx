    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
      <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
        <div className="mb-1 flex items-center justify-between">
          <h3 className="text-base font-bold text-slate-900">Quyền kho — {employee.full_name}</h3>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-700"><X className="h-4 w-4" /></button>
        </div>
        <p className="mb-4 text-xs text-slate-500">Chỉ áp dụng trong nhóm kho đang chọn. Tick quyền nào thì nhân sự này được thao tác đúng quyền đó.</p>
        <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
          {allCodes.map(code => (
            <label key={code} className="flex items-center gap-2.5 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-700 hover:bg-slate-100">
              <input type="checkbox" checked={granted.has(code)} onChange={() => onToggle(code)} className="accent-blue-500" />
              {WAREHOUSE_PERMISSION_LABELS[code] || code}
            </label>
          ))}
        </div>
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-600 hover:text-slate-700">Hủy</button>
          <button onClick={onSave} disabled={saving} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-700 disabled:opacity-50">
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex w-full items-center rounded-xl border border-slate-200 bg-white px-3 py-2.5 lg:w-96 focus-within:border-blue-500/50">
          <Search className="h-4 w-4 shrink-0 text-slate-500" />
          <input
            value={searchTerm}
            onChange={event => setSearchTerm(event.target.value)}
            placeholder="Tìm tên, vai trò, Telegram ID hoặc nhóm…"
            className="ml-2 w-full bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-500"
          />
        </div>
      </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="flex min-h-16 flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <h3 className="flex items-center gap-2 text-base font-bold text-slate-900">
            ...
            {loading ? (
              ...
              <div className="divide-y divide-slate-100">
                {filteredStaff.map((user, index) => {
                  ...
                  return (
                    <div key={user.id} className={`grid ${ROW_GRID} min-h-[92px] items-center gap-4 px-5 py-3 transition hover:bg-slate-50`}>
                      ...
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          {isEditing ? (
                            <input value={editForm.full_name} onChange={event => setEditForm({ ...editForm, full_name: event.target.value })} className="w-full rounded-lg border border-blue-500/40 bg-slate-50 px-2.5 py-1.5 text-sm text-slate-900 outline-none" />
                          ) : (
                        ...
                        {isEditing ? (
                          <>
                            <select
                              ...
                              className="w-full rounded-lg border border-blue-500/40 bg-slate-50 px-2.5 py-1.5 text-xs text-slate-900 outline-none"
                            >
                            ...
                            {!STAFF_ROLES.includes(editForm.role) && (
                              <input
                                ...
                                className="mt-1.5 w-full rounded-lg border border-blue-500/40 bg-slate-50 px-2.5 py-1.5 text-xs text-slate-900 outline-none"
                              />
                            )}
                          </>
                        ) : <Tag tone="cyan">{user.role || 'Chưa có vai trò'}</Tag>}
                        <p className="line-clamp-2 text-[11px] leading-4 text-slate-500" title={user.group_name || 'Chưa thuộc nhóm'}>{user.group_name || 'Chưa thuộc nhóm'}</p>
                      </div>

                      <div className="flex flex-wrap items-center gap-2">
                        {isEditing ? (
                          <>
                            <label className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-[11px] text-slate-600">
                              Phép <input type="number" min="0" value={editForm.leave_quota} onChange={event => setEditForm({ ...editForm, leave_quota: Number(event.target.value) || 0 })} className="w-10 bg-transparent text-center font-bold text-slate-900 outline-none" /> ngày
                            </label>
                            <label className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2 py-1.5 text-[11px] text-slate-600">
                              <input type="checkbox" checked={editForm.is_exempt_checkin} onChange={event => setEditForm({ ...editForm, is_exempt_checkin: event.target.checked })} className="accent-blue-500" />Miễn check-in
                            </label>
                            <label className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-2 py-1.5 text-[11px] text-slate-600">
                              <input type="checkbox" checked={editForm.need_report} onChange={event => setEditForm({ ...editForm, need_report: event.target.checked })} className="accent-blue-500" />Cần báo cáo
                            </label>
                          </>
                        ) : (
                          <>
                      ...
                      <div className="flex flex-col items-start gap-2">
                        {isEditing ? (
                          <select value={editForm.is_active ? 'active' : 'disabled'} onChange={event => setEditForm({ ...editForm, is_active: event.target.value === 'active' })} className="rounded-lg border border-blue-500/40 bg-slate-50 px-2.5 py-1.5 text-xs text-slate-900 outline-none">
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-2">
                        {isEditing ? (
                          <>
                            ...
                            <button onClick={cancelEdit} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-500 hover:text-slate-700"><X className="h-3.5 w-3.5" />Hủy</button>
                          </>
                        ) : (
                          <>
                            {canPause && (
                            ...
                            <button onClick={() => startEdit(user)} className="inline-flex items-center gap-1 rounded-lg border border-blue-500/25 bg-blue-500/10 px-3 py-2 text-xs font-semibold text-blue-400 hover:bg-blue-500/20"><Edit3 className="h-3.5 w-3.5" />Sửa</button>
