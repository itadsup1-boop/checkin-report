...
export default function LeaveManagement({ selectedGroupId = 'ALL' }) {
  ...
  const getRequestTypeLabel = (type) => {
    switch (type) {
      ...
      default:
        return <span className="px-2.5 py-1 bg-slate-500/10 text-slate-500 rounded-md text-xs border border-slate-500/20">{type}</span>;
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight mb-2">Xin Nghỉ & Quỹ Phép</h2>
          <p className="text-slate-500 text-sm">Quản lý và duyệt đơn xin nghỉ phép, theo dõi quỹ phép năm của nhân viên</p>
        </div>
        <div className="flex items-center bg-white rounded-full px-4 py-2 border border-slate-100 w-full md:w-80 transition-all focus-within:border-blue-500/50 focus-within:ring-1 focus-within:ring-blue-500/50">
          <Search className="w-5 h-5 text-slate-500" />
          <input
            type="text"
            placeholder="Tìm theo tên, vai trò, lý do..."
            className="bg-transparent border-none outline-none text-sm ml-3 w-full text-slate-900 placeholder-slate-500"
            value={searchTerm}
      ...
      {activeSubTab === 'requests' ? (
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {loading ? (
                  ...
                  filteredRequests.map((req, idx) => {
                    const colors = ['from-blue-500 to-blue-500', 'from-emerald-500 to-green-500', 'from-purple-500 to-pink-500', 'from-amber-500 to-orange-500'];
                      ...
                      <tr key={req.id} className="hover:bg-slate-50 transition-colors group">
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-semibold text-slate-900">{req.full_name || 'N/A'}</p>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex flex-col gap-1 items-start">
                            ...
                            {req.request_type === 'LATE' && req.late_minutes && (
                              <span className="text-[10px] text-slate-500 font-mono mt-0.5 flex items-center gap-0.5">
                                <Clock className="w-3.5 h-3.5" /> Muộn {req.late_minutes} phút
                              </span>
                            )}
                          </div>
                        </td>
                        <td className="py-4 px-6 text-slate-600 max-w-[200px] truncate" title={req.reason}>
                          {req.reason || '—'}
                        </td>
                        <td className="py-4 px-6 text-center">
                          {req.proof_url ? (
                            <button
                              ...
                              className="px-2.5 py-1.5 bg-blue-500/10 text-blue-400 rounded-md text-xs border border-blue-500/20 hover:bg-blue-500/20 transition-colors inline-flex items-center gap-1"
                            >
                          ...
                          <div className="flex items-center gap-1.5 justify-end">
                            ...
                            {!isPending && (
                              <button
                                ...
                                className="px-2.5 py-1.5 bg-slate-500/10 hover:bg-slate-500/20 text-slate-500 rounded-lg text-xs font-semibold transition-all border border-slate-500/20 flex items-center gap-1"
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="text-sm font-medium text-slate-500">Chọn năm làm việc:</span>
            <select
              value={selectedYear}
              onChange={e => setSelectedYear(parseInt(e.target.value))}
              className="bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 text-sm font-medium focus:outline-none focus:border-blue-500/50"
            >
                ...
                <tbody className="divide-y divide-slate-100 text-sm">
                  {loading ? (
                    ...
                    filteredBalances.map((bal, idx) => {
                      ...
                      return (
                        <tr key={bal.user_id} className="hover:bg-slate-50 transition-colors">
                          <td className="py-4 px-6 text-slate-500 font-medium">{idx + 1}</td>
                          <td className="py-4 px-6">
                            <div className="flex items-center gap-3">
                              <div className={`w-9 h-9 rounded-full bg-gradient-to-tr ${gradientClass} flex items-center justify-center text-white font-bold text-xs shrink-0`}>
                              ...
                              <div>
                                <p className="font-semibold text-slate-900">{bal.full_name || 'N/A'}</p>
                                <p className="text-xs text-slate-500">{bal.telegram_id}</p>
                              </div>
                            </div>
                          </td>
                          <td className="py-4 px-6 text-slate-600">
                            <span className="px-2.5 py-1 bg-slate-50 rounded-md text-xs border border-slate-100">{bal.role || 'N/A'}</span>
                          </td>
                          ...
                          <td className="py-4 px-6 text-center">
                            <span className={`px-2 py-0.5 rounded font-bold ${remaining < 0 ? 'text-rose-400' : 'text-blue-400'}`}>
                              {remaining} ngày
                            </span>
                          </td>
                          <td className="py-4 px-6 text-right">
      ...
      {selectedProof && (
        <div className="fixed inset-0 bg-slate-50/90 backdrop-blur-md z-50 flex flex-col items-center justify-center p-4">
          <div className="absolute top-4 right-4 flex gap-4">
            <button
              onClick={() => setSelectedProof(null)}
              className="p-3 bg-slate-50 hover:bg-slate-100 text-slate-900 rounded-full border border-slate-200 hover:scale-105 transition-all"
              title="Đóng"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
          <div className="max-w-4xl max-h-[80vh] overflow-hidden rounded-2xl border border-slate-200 shadow-2xl relative bg-white">
            <img
              src={selectedProof}
          ...
          <p className="text-slate-500 text-sm mt-4 italic">Minh chứng đính kèm đơn xin nghỉ</p>
        </div>
      )}
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 bg-white border border-blue-500/30 shadow-2xl shadow-blue-500/20 text-slate-900 px-6 py-4 rounded-xl flex items-center gap-3 z-50 animate-[fadeIn_0.3s_ease-out]">
          <AlertCircle className="w-5 h-5 text-blue-400 animate-bounce" />
