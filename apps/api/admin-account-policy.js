...
export const RETAIL_MANAGER_ROLE = ADMIN_ROLES.RETAIL_MANAGER;
...
export async function validateAssignedGroupsForRole({ pool, role, assignedGroups }) {
    ...
    if (role === ADMIN_ROLES.WAREHOUSE_ACCOUNTANT) {
        if (!groupIds.length) {
    ...
    if (role === ADMIN_ROLES.RETAIL_MANAGER) {
        if (!groupIds.length) {
            throw Object.assign(new Error('Tài khoản Quản lý thị trường phải được gán ít nhất một nhóm check-in thị trường.'), {
                status: 400
            });
        }
        ...
        const validIds = new Set(result.rows.map(row => String(row.telegram_group_id)));
        if (groupIds.some(groupId => !validIds.has(groupId))) {
            throw Object.assign(new Error('Quản lý thị trường chỉ được gán vào các nhóm Telegram có vai trò Check-in thị trường.'), {
                status: 400
            });
        }
        return groupIds;
