...
import dns from 'dns';
import { checkImageWithExternalService } from './external_validator.js';
...
import { getKpiDocForGroup, getCustomerDocForGroup, getGroupIdFromTelegramId } from './sheetManager.js';
...
async function logPenaltyToSheet(user_full_name, employee_code, telegram_id, penalty_type, amount, details) {
    if (amount <= 0) return;
    const telegram_group_id = await getGroupIdFromTelegramId(telegram_id);
    const doc = await getKpiDocForGroup(telegram_group_id);
    if (!doc) return;
            ...
            if (group.remind_time_1) {
                ...
                if (currentTimeString === penaltyTimeString) {
                    ...
                    if (missing.length > 0) {
                        ...
                        const doc = await getKpiDocForGroup(group.telegram_group_id);
                        if (doc) {
                            sheetQueue = sheetQueue.then(async () => {
                                try {
                                    await doc.loadInfo();
...
async function processReport(user, parsedJSON, kpiTarget, telegram_id, group_id, text, ctx, botInstance = null, debt_info = null) {
    try {
        ...
        const doc = await getKpiDocForGroup(group_id);
        if (doc) {
        sheetQueue = sheetQueue.then(async () => {
            try {
                    await doc.loadInfo();
        ...
        if (user && user.id) {
            ...
            if (photoArray && photoArray.length > 0) {
                ...
                try {
                    ...
                    const checkResult = await checkImageWithExternalService(arrayBuffer);

                    if (checkResult && checkResult.status === 'ACCEPTED') {
                        // Lưu lại reference vào database
                        await pool.query(
                            `INSERT INTO image_fingerprints (employee_id, telegram_file_id, external_image_id, report_date)
                             VALUES ($1, $2, $3, CURRENT_DATE)`,
                            [user.id, file_id, checkResult.image_id]
                        );
                    } else if (checkResult && (checkResult.status === 'BLOCKED_DUPLICATE' || checkResult.status === 'BLOCKED_EXACT' || checkResult.status === 'REVIEW')) {
                        // Lấy thông tin ảnh gốc từ DB
                        const matchedId = checkResult.matched_media_info ? checkResult.matched_media_info.id : checkResult.matched_candidate_id;
                        if (matchedId) {
                            const oldPicRes = await pool.query(
                                `SELECT f.telegram_file_id, f.created_at, e.full_name as employee_name
                                 FROM image_fingerprints f
                                 LEFT JOIN employees e ON f.employee_id = e.id
                                 WHERE f.external_image_id = $1 LIMIT 1`,
                                [matchedId]
                            );
                            
                            if (oldPicRes.rows.length > 0) {
                                const oldPic = oldPicRes.rows[0];
                                const dateStr = new Date(oldPic.created_at).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' });
                                let warnMsg = `🚨 <b>PHÁT HIỆN NGHI VẤN XÀI LẠI ẢNH CŨ</b> 🚨\n`;
                                warnMsg += `👤 Nhân viên gửi: <b>${user.full_name}</b>\n`;
                                warnMsg += `⚠️ Ảnh gửi lên trùng lặp với ảnh của <b>${oldPic.employee_name}</b> nộp lúc ${dateStr}.\n`;
                                ...
                                await bot.telegram.sendMediaGroup(ctx.chat.id, [
                                    { type: 'photo', media: oldPic.telegram_file_id, caption: `BẢN GỐC của ${oldPic.employee_name} nộp ${dateStr}` },
                                    { type: 'photo', media: file_id, caption: `BẢN MỚI do ${user.full_name} gửi lên` }
                                ]);
                            }
                        }
                    }
                } catch (checkErr) {
                    console.error("Lỗi check ảnh external gửi trực tiếp:", checkErr);
                }
            } else if (videoObj) {
...
bot.command(['app', 'form', 'lamviec', 'tienich', 'start'], (ctx) => {
    ...
    // URL to Native Web App inside the group!
    const dmUrl = `https://t.me/${botUsername}/form?startapp=baocao_${ctx.chat.id}_${Date.now()}`;
    const scheduleUrl = `https://t.me/${botUsername}/form?startapp=schedule_${ctx.chat.id}_${Date.now()}`;
...
bot.action('CHECK_UPDATE_REPORT', async (ctx) => {
    ...
    try {
        ...
        if (hasReport) {
            const botUsername = ctx.botInfo.username;
            const dmUrl = `https://t.me/${botUsername}/form?startapp=baocao_${groupId}_${Date.now()}`;
...
bot.start((ctx) => {
    ...
    if (text.startsWith('/start baocao_') || text.startsWith('/start schedule_')) {
        ...
        const finalUrl = isSchedule ? `${miniAppUrl}/mini-app/schedule.html?chat_id=${groupId}&v=${Date.now()}` : `${miniAppUrl}/mini-app/form.html?chat_id=${groupId}&v=${Date.now()}`;
...
botApp.get('/api/bot/get-report-today', async (req, res) => {
...
botApp.get('/api/schedules', async (req, res) => {
    try {
        const { date, telegram_id } = req.query; // YYYY-MM-DD
        let { groupId } = req.query;

        if (!groupId && telegram_id) {
            const eRes = await pool.query('SELECT telegram_group_id FROM employees WHERE telegram_id = $1 LIMIT 1', [telegram_id]);
            if (eRes.rows.length > 0) groupId = eRes.rows[0].telegram_group_id;
        }
    ...
    try {
        ...
        const result = await pool.query(
            `SELECT id, employee_name, customer_name, phone, service, sessions, appointment_time, status, cancel_reason 
             FROM customer_appointments 
             WHERE phone ILIKE $1 AND (group_id = $2 OR group_id IS NULL OR $2 = '')
...
async function checkOverlap(appointmentTimeStr, groupId, excludeId = null) {
    const query = `
        ...
        WHERE status = 'ACTIVE' 
        AND group_id = $1
        AND appointment_time BETWEEN ($2::timestamp - INTERVAL '59 minutes') AND ($2::timestamp + INTERVAL '59 minutes')
        ${excludeId ? 'AND id != $3' : ''}
        LIMIT 1
    `;
    const params = excludeId ? [groupId, appointmentTimeStr, excludeId] : [groupId, appointmentTimeStr];
...
botApp.post('/api/schedules/add', async (req, res) => {
    try {
        const { initData, customer_name, phone, service, sessions, revenue, appointment_time, is_urgent, groupId: reqGroupId } = req.body;
        ...
        // Priority: 1. Start param (inline button), 2. req.body (app URL), 3. MINI_APP fallback
...
botApp.post('/api/schedules/edit', async (req, res) => {
    try {
        ...
        const dbResGroupId = await pool.query('SELECT group_id FROM customer_appointments WHERE id = $1', [id]);
        if (dbResGroupId.rows.length === 0) return res.json({success: false, error: "Not found"});
        const groupId = dbResGroupId.rows[0].group_id;

        // Check overlap excluding this id
        const overlap = await checkOverlap(appointment_time, groupId, id);
        if (overlap) {
            const timeOverlap = new Date(overlap.appointment_time).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' });
            return res.json({
                success: false,
        ...
        const dbRes = await pool.query(
            `UPDATE customer_appointments 
             ...
             WHERE id = $4 RETURNING sheet_row_index, employee_name, group_id, group_id`,
...
botApp.post('/api/schedules/cancel', async (req, res) => {
    try {
        ...
        const dbRes = await pool.query(
            `UPDATE customer_appointments SET status = 'CANCELLED', cancel_reason = $1 WHERE id = $2 RETURNING sheet_row_index, employee_name, group_id, group_id`,
            [cancel_reason, id]
        );
        const rowIndex = dbRes.rows[0]?.sheet_row_index;
        const empName = dbRes.rows[0]?.employee_name;
...
botApp.post('/api/bot/submit-report', async (req, res) => {
    try {
...
cron.schedule('2 20 * * *', async () => {
    try {
        ...
        for (const g of groupsRes.rows) {
            ...
            await bot.telegram.sendMessage(g.group_id, msg, { parse_mode: 'HTML' }).catch(() => {});
        }
    } catch (e) {
...
bot.on('photo', async (ctx) => {
    try {
        ...
        const rowIndex = apt.sheet_row_index;
        const empName = apt.employee_name;
        const customerDoc = await getCustomerDocForGroup(apt.group_id);
        if (rowIndex && customerDoc) {
            customerSheetQueue = customerSheetQueue.then(async () => {
                await customerDoc.loadInfo();
                const sheet = customerDoc.sheetsByTitle[empName];
